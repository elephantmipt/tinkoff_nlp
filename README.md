...
Понятно, что по началу слова легче угадать его конец, да и за верный ответ для bpe нужно считать угадывание всех подслов слова.
...
Пусть в слове допущена ошибка, в таком случае модель bpe, которая ни разу не видела такого слова, сначала, при малом количестве ошибок, всего скорее все же
разобъет наше слово на n-грамму.

Ну например, пусть у нас было слово `благополучный`, пусть в нем допущена ошибка и
`благополучный` —> `блааополучный`. Если мы имеем дело с модели без bpe, то она просто присвоит этому 
токену `[UKNOWN]` то есть `благополучный` —> `блааополучный` —> `[UKNOWN]`.

В случае с bpe при достаточной удаче мы можем получить что-то вроде
`благополучный` —> `блааополучный` —> `бла` + `[UNKNOWN]` + `пол` + `уч` + `ный`.
Слово `благополучный` до сих пор угадывается и модель, наверное, сможет предсказать следующее слово.

Так же анализ редко встречающихся слов и ошибок в них был разобран [здесь](https://www.aclweb.org/anthology/P16-1162.pdf).
...
````
 ...
 Есть скрипт `run.sh`. Кнопка «сделать хорошо». 
